...
import 'file:///D:/AndroidStudioProjects/earthsite/lib/bean/return_body_entity.dart';
...
///
class HttpRequest {
  static String _baseUrl = Urls.Alarm_url;
...
//      contentType: ContentType.json,
    ...
    if (null != response.data) {
      ...
      if (null != returnBodyEntity) {
        switch (returnBodyEntity.Code) {
          ...
          case 400:
            errorCallBack(
                returnBodyEntity.Code, returnBodyEntity.Msg);
          /*  if (null != context) {
              Navigator.of(context).push(
                new MaterialPageRoute(
                  builder: (context) {
                    return new Scaffold(
                      body: new LoginPage_sec(),
                    );
                  },
                ),
              );
  ...
  get_otherInfo(url,
    ...
    if (null != response.data) {
//      Map<String,dynamic> dataJson=json.decode(response.data);
//      List responseJson = json.decode(json.encode(dataJson));

      if (null != json.decode(response.data)) {
        switch (json.decode(response.data)["code"]) {
          case 200:
            successCallBack(json.decode(response.data));
            break;
          case 500:
            errorCallBack(json.decode(response.data)["code"]);
          /*  if (null != context) {
              Navigator.of(context).push(
                new MaterialPageRoute(
                  builder: (context) {
